...
%\item As the imaginary part of the FS should be also localized below threshold this implies that also the excitations are localized quantities when E is below threshold
...
%\item The absorption spectra, which are only dependent by the imaginary part of R, are therefore well expressed only for discrete energies
...
\textbf{Theorem:} Only the eigenstates with negative energy might have a finite-size correction that can be expressed perturbatively.
...
\ee
% {\cal R}_{\op O}(\omega) = 
%  \ii \sum_p\left(\bra{\psi_p}\op A(\omega)\op Q_0\op O\ket{\psi_p} - \bra{\psi_p}\op O\op Q_0\op A(\omega)\ket{\psi_p}\right) =\\ =
% \ii \sum_p\left(\bra{\psi_p} \op \Phi(\omega) \op G_0^-(\epsilon_p - \bomega)  \op Q_0 \op O\ket{\psi_p} - 
% \bra{\psi_p}\op O\op Q_0\op G_0^+(\bomega + \epsilon_p) \op \Phi(\omega) \ket{\psi_p}\right) = \\ 
% % \ii \int \dd \omega' \sum_p\left(\frac{\bra{\psi_p} \op \Phi(\omega) \op B(\omega')  \op Q_0 \op O\ket{\psi_p}}{\epsilon_p - \bbomega - \omega'} - 
% % \frac{
% % \bra{\psi_p}\op O\op Q_0\op B(\omega') \op \Phi(\omega) \ket{\psi_p}}
% % {\epsilon_p + \bbomega - \omega'}\right) = \\ 
% \;.
