...
functional space. Therefore, together with the discrete part of its spectrum $\eps_p$, the Hamiltonian may have an essential spectrum 
$\eps(\alpha)$, parametrized by the continuum label $\alpha$. 
...
$\ket{\psi_0} = e^{-\\i \eps_p t_0}\ket{\psi_p}$, we can write the time evolution of a (discrete) eigenvector of $\hnot$ as:
\be \lb{ketTimeEvol3} 
\ket{\psi_p(t)} = e^{-\ii \eps_p t}\left( \identity - \ii \intdef{t_0}{t}{t'} \op{U}_0(t-t') \op\Phi^{a.l,s.l}(t') \op{U}_0(t'-t)  \right)\ket{\psi_p} \; ,
\ee
...
\ee
so that the effect of the switch-on protocol are completely reabsorbed in the perturbing field. In the subsequent analysis we will omit the apex in the fields $\Phi$, 
...
\ket{\psi_p(t)} = e^{-\ii \eps_p t}\left( \identity - \ii \op A(t)  \right)\ket{\psi_p} \; , \qq 
\bra{\psi_p(t)} = \bra{\psi_p} \left( \identity + \ii \op A(t)  \right) e^{\ii \eps_p t} \; .
\ee

\subsection{Linear response on the density operator. Liouville equation in the time domain}

We introduce the \emph{density operator} defined as the sum of the densities of all the occupied states having energy $\epsilon_p$ lower than the \emph{chemical potential} 
(Fermi energy):
\be\label{rhodef1}
\dmnot = \sum_p^{n_{occ}} \ket{\psi_p} \bra{\psi_p}  \;. 
\ee
...
\be\lb{L0sopDef1}
\sop{\mathcal L_0} \equiv \commutator{\hnot}{\cdot} \;,
\ee
so that the Liouville equation can be written as:
\be\lb{LiouvilleRhoDef2}
\ii \d_t \dm'(t) = \sop{\mathcal L_0} \dm'(t) + \commutator{\op\Phi(t)}{\dmnot} \;.
\ee
...
In this case the perturbing field $\Phi$ has also the effect of adding a further perturbation operator to the perturbing Hamiltonian. In the linear response framework, 
...
\ii \d_t \dm'(t) = \sop{\mathcal L_0} \dm'(t) + \commutator{\op V'(t)}{\dmnot} + \commutator{\op \Phi(t)}{\dmnot} \;.
\ee
...
\ii \d_t \dm'(t) = \sop{\mathcal L} \dm'(t) + \commutator{\op \Phi(t)}{\dmnot} \;, \qq
\sop{\mathcal L} = \sop{\mathcal L_0} + \sop{\mathcal K} \;.
\ee
...
\ee
The action of $\sop{\mathcal L_0}$ on $\dm'(\omega)$ follows from \eqref{L0sopDef1}:
\be\label{LiouvillianF'def1}
\sop{\mathcal L_0} \dm'(\omega) = \ii \commutator{\hnot}{\commutator{\dmnot}{\op A(\omega)}} = 
\ii \commutator{\dmnot}{\commutator{\hnot}{\op A(\omega)}}
\;,
\ee
\be\label{eoma}
\commutator{\hnot }{\op A(\omega)} = \bar{\bar\omega} \op A(\omega) - \ii \op \Phi(\omega)\;.
\ee
where $\bar{\bar\omega} = \omega + 2\ii\eta$
