...
\be
\left(k_p(\omega)^2+\ii\eta + \nabla^2\right)f_p(\omega,\r) = s(\omega,\r) \;\;, {\rm with} \;\; k_p(\omega)^2 = 2(\omega+\eps_p) \;,
\ee
...
f_p(\omega,\r) \sim \frac{e^{\ii \sqrt{k_p(\omega)^2} |\r|}}{|\r|} \;,
\ee
so the asymptotic character of the fluctuation states depends on the sign of $k_p(\omega)^2$. In particular equation \eqref{fluctuationStateIntEq2} describes an
...
\ee
The quality of the basis can be evaluated by the requirement that the numerical coefficients appearing in the r.h.s provide a reliable reconstruction of the formal scalar product. 
Here we are assuming that the ground state properties of the unperturbed system have been computed in the same basis introduced above. This means that the feature of the basis, 
that for instance for real space basis are given by the grid spacing and number of points (that determines the dimension simulation box), are chosen so that the coefficients
$\brket{\mu}{\psi_p}$ represent a valid discretization of the occupied orbitals. The same can be stated also for the matrix elements $\bra{\mu}\op O\ket{\psi_p}$, by assuming 
that $\op O$ acts as a local operator on the occupied orbitals. 

The most interesting point of this discussion concerns the assessment of the reliable sampling of the fluctuation states through the coefficients $\brket{f_p(-\omega)}{\mu}$ and
...
regime. To check this point we can compute the perturbation-induced modification of an observable an verify that its value scales linearly with the magnitude of the field}. %, without  

%In order to address this aspects it is possible to assess the reali
%We propose an operative procedure to perform a direct measurement of the  

%An explicit check can be performed at $\omega=0$.  



%This condition implies that the coefficients
%$\brket{\mu}{\psi_p}$ represent a valid discretization of the occupied orbitals and also the matrix elements $\bra{\mu}\op O\ket{\psi_p}$ can assumed to be well sampled, assuming that
%$\op O$ acts as a local operator on the occupied orbitals. 
