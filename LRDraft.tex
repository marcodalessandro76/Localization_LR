...
\subsection{Localization properties of the response density}

We present an ulterior expression of the response density operator in the frequency space with the aim to establish its (spatial) localization properties.
The starting point of this analysis is the expression of the response density in terms of the commutator of \eqref{rhoPrimeOmegaDef1}. We observe that, for any 
operator $\op O$, we may write:  
\be
\commutator{\dmnot}{\op O} = \dmnot \op O \op Q_0 - \op Q_0 \op O \dmnot\;,
\ee
where $\op Q_0= \identity - \dmnot$ projects the operator in the subspace orthogonal to $\dmnot$. So, thanks to this relation we have that:
\be
...
\ee
where the $\omega$ dependence of the bra is consistent with hermitian conjugation property of the Fourier transform of a hermitian operator. Fluctuation states are, by definition,
orthogonal to the occupied eigenstates of $\hnot$, so that:
\be
...
\ee
Applying the Liouville equation \eqref{LiouvillianRhopomegaDef3} to \eqref{rhoPrimeFluctuationStateDef1} allows us to derive a set of equation of motion for the
fluctuation states. The following equations are useful for this task:
\begin{align}
\Liouvnot\dm'(\omega) & = \sum_p\left(\ketbra{\psi_p}{f_p(-\omega)}\left(\eps_p-\hnot\right) + \left(\hnot-\eps_p\right)\ketbra{f_p(\omega)}{\psi_p}\right) \;, \nn \\
...
\bra{f_p(\omega)}\left[\bbomega + \left(\eps_p-\hnot\right)\right] &= \bra{\psi_p}\left(\op V'(-\omega) + \op \Phi(-\omega)\right)\op Q_0 \;.
\end{align}
