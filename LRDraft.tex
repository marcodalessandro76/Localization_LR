...
E \op{\tilde E} \;.
\end{align}
The introduction of the excitation operators allows to provided a complete operator version of equation \eqref{RhopExcitationTransBasisDef2}
...
We introduce the excitonic states defined via the linear combination of the basis set orthogonal to $\dmnot$ with the
...
\ee
The operator eigenvalue equations \eqref{opEeigenvalueDef1} and \eqref{optildeEeigenvalueDef1} can be translated in Schroedinger-like equations
for the excitonic states. 

\vspace{0.5cm}
\emph{Solution for $\coupl = 0$}
\vspace{0.5cm}
...
\ee
and projecting on $\ket{\psi_q}$ provides the eigenvalue condition for the excitonic $\ket{\phi^E_p}$ state:
\be
\left(\hnot - \eps_p\right)\ket{\phi^E_p} = E \ket{\phi^E_p} \;.
\ee
In the same way we can derive an analogous equation for $\tbra{\phi^E_p}$ by using the eigenvalue condition for $\op{\tilde E}$. To achieve this
...
\ee
In other terms we find that the perturbation states must be left and right eigenvectors of  $\hnot$. 

The operators $\op E$ and $\op{\tilde E}$ have an important physical meaning:
if they are used as perturbation operators they would generate a response density 
with a well-defined energy:
\be
\commutator{\op E}{\dmnot} = E \;, \qquad \commutator{\op{\tilde E}}{\dmnot} = -\op{\tilde E}
\ee
The above equations show that we would have, respectively:
\be
\dm'(\omega) = \frac{1}{\bbomega - E} \op E\;,\qquad
\dm'(\omega) = \frac{1}{\bbomega + E} \op{\tilde E}
\ee
In other terms, they constitute the \emph{resonant channels} of the excitation of the
density. The decomposition of the perturbation operator $\op \Phi$ among these channels
would provide $\dm'(\omega)$.
