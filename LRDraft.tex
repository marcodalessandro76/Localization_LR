...
%For instance the first addend of \eqref{LinearResponseFunctDef2} reads: 
...
%On the contrary, the most interesting point of this discussion concerns with the assessment of the 
%reliable sampling of the fluctuation states through the coefficients $\brket{f_p(-\omega)}{\mu}$ and $\brket{\mu}{f_p(\omega)}$. The problem turns out to be highly is non-trivial 
%since the behavior of the fluctuation states strongly depends on $\omega$, as will be readily clarified in the next section.


%$\brket{f_p(-\omega)}{\mu}$ and $\brket{\mu}{f_p(\omega)}$ in the domain in which the matrix elements $\bra{\mu}\op O\ket{\psi_p}$ (and its hermitian 
%conjugates) are not vanishing. 
%The task of the linear response formalism to provide reliable results for the expectation value of $\op O$ can be reformulated in the problem of finding an accurate and unbiased
%expression for the coefficients $\brket{f_p(-\omega)}{\mu}$ and $\brket{\mu}{f_p(\omega)}$ in the domain in which the matrix elements $\bra{\mu}\op O\ket{\psi_p}$ (and its hermitian 
%conjugates) are not vanishing. 


% We assume that the ground state problem has been already solved and that the occupied orbitals  introduce a computational basis $\{\ket{\mu}\}$, composed by a \emph{finite}
% set of functions
% 
% Here we are dealing with a linear response problem and 
...
% of the formal scalar product. 

% Here we are assuming that the ground state properties of the unperturbed system have been computed in the same basis introduced above. This means that the 
...
% aggiungere un preambolo a quello che segue. Soluzione usando le equazioni di Sternheimer e approccio con le eccitazioni....
...
\subsubsection{Criterion for the choice of the computational basis} %underlying
...
\be\lb{psiAlphaDef1}%{orthonormalityDef1}
...
%both with and without the explicit time-independent perturbing field. 
%Denoting with $n_{occ}$ the number of occupied orbitals  
