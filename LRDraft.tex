...
Possible choice of the switching on protocol are, for instance, the \emph{adiabatic limit} and the \emph{sudden limit} case. In the first
...
$e^{-\eta_a |t|}$. In the second protocol the perturbation is abruptly switched on at the finite time $t_0$, so that the time structure of
$\hone{}$ is multiplied times the $\theta$-function $\theta(t-t_0)$.

It is possible to write:
\be \lb{ketTimeEvol3} 
\ket{\psi_p(t)} = e^{-\ii E_p t}\left( \identity - \ii \op A(t_0,t)  \right)\ket{\psi_p} \; ,
\ee
where: 
\be
\op A(t_0,t) = \intdef{t_0}{t}{t'} \op{U}_0(t-t') \hone{t'} \op{U}_0(t'-t)
\ee
we can write:
\be
