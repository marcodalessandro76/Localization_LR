...
\begin{abstract}
A reliable procedure for the analysis of a system within the framework of the linear response should include
the study of the conditions under which numerical convergence of the results can be achieved. 
...
we explore the interplay between locality of the perturbed wavefunctions and the 
numerical convergence of observable quantities. We identify some convergence indicators and discuss under which regimes they might be employed. This will enable us to draw some considerations on the analytic structure of the linear susceptivity for open systems and on the potential convergence issues that might arise in a given computational setup.
...
Any local operator, like \emph{e.g.} a static electric field $\mathbf F$, with associated scalar potential $\Phi_s(\r) = -\mathbf F \cdot \r$, would satisfy such property.
We consider the perturbed hamiltonian $\hnot[\dm_{\Phi_s}] + \op\Phi_s$, where $\dm_{\Phi_s}$ represents the GS density of the perturbed system, and assume that the 
...
with a potential local behavior only for $\omega$ below the ionization threshold. So, Comparing fluctuation states among diverse numerical treatments would be a 
challenging task.
...
% \end{align}
\be
\ket{f_p^\Phi(\omega)} = \ket{f^{\Phi}_p(\omega)}_D + \ket{f^{\Phi}_p(\omega)}_C \lb{FSExcitationDef1} \;,
\ee
with
\begin{align}
 \ket{f_p^\Phi(\omega)}_D &= \sum_a^{\Omega_a < |\eps_a|} \ket{w_p^a} \frac{\varphi_p^a(\op \Phi,\omega)}{\omega^2-\Omega_a^2} \;, \nn \\
 \ket{f_p^\Phi(\omega)}_C &= \int_{\Omega_a > |\eps_a|} \!\!\!\!\dd a \ket{w_p^a} \frac{\varphi_p^a(\op \Phi,\omega)}{\omega^2-\Omega_a^2} \;, \nn
\end{align}
...
For \emph{any} value of $\omega$ the ket $\ket{f_p^\Phi(\omega)}_D$ has, by definition, a bound-state like
...
For this to be true, the contribution from the delocalized excited states defining
$\ket{f_p^\Phi(\omega)}_C$ should resum into a localized, asymptotically vanishing state.
Indeed, there is no particular reason to believe that $\ket{f_p^\Phi(\omega)}_C$ should be
zero. The linear independence of the $\ket{w_p^a}$ states would imply that 
...
We \emph{cannot} exclude that a fluctuation state, even if genuinely localized, will have a nonzero projection
...
2 \mathrm{Re}\left(\bra{\psi_p} \op O \ket{f_p^\Phi(\omega)}_{D,C} \right)\;,
\ee
...
LR quantities are in general unreliable for absorption spectra above IP, whilst provide precise values
