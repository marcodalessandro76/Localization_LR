...
The structure of the excitation spectrum follows from the properties of the excited states described above. To clarify this point we consider a value of $E$ for which equations 
\eqref{ExcitationOperatorsDef1} admit a solution and, consequently, equations \eqref{ExcitationOperatorsDef3} possess a solution where \emph{at least} one of the excited states is non vanishing. 
According to \eqref{ExcitationOperatorsDef2}, $\op E$ is expressed in terms of these states and we denote with $p=p_m$ the label of the non vanishing $\ket{\phi^E_p}$ state with the highest value 
of $|\eps_p|$. From the previous considerations it emerge that if $E<|\eps_{p_m}|$ \emph{all} the excited states that contribute to the dyadic representation of the excitation operator possess a 
bound-state like behavior. Excitations of this type thus represent localized quantities and the associated eigenvalue are discrete. On the contrary, for $E>|\eps_{p_m}|$, excitations have a 
delocalized character and the corresponding eigenvalues belong to a continuum of accessible values.

The typical structure of the excitation spectrum is represented in figure \ref{ExcitationLandscape}. 
Excitations are separated into two categories (add comment on the observable character of localized excitations). 
The plot evidences that below the first ionization energy $|\eps_h|$ the spectrum is purely composed by a discrete set of localized excitations. Instead, in the intermediate energy interval that extends 
from $|\eps_h|$ to the absolute value of the deepest occupied orbital, the discrete sector of the spectrum is embedded in a continuum of excitations delocalized in the whole accessible space. Lastly,
for energies higher than $|\eps_1|$ only delocalized excitations are present. 
...
\subsection{Comments. Physical meaning of the excitation spectrum}

True poles vs spectral function....
