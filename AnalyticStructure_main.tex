...
Thus the condition $\dm'_\Phi = \dm_{\perp}$ holds.
...
\ee
with the state%:here in this case the source term contains further addend apart from the unperturbed potential and reads:
$
...
%\footnote{The fulfilling of Eq.~\eqref{SEHelmholtz1} for energies below threshold
...
%The quantities $\braket{\psi_p \op O| f_p^\Phi(\omega)}$, 
%being scalar products of bound-state-like objects, 
...
%since $\ket{f_p^\Phi(\omega)}$ becomes an unbound, delocalized function, 
...
\end{align}
For each choice of $p,a$, these objects represent, respectively, the state in which $\ket{\psi_p}$
...
% $\ket{\psi_p}$ 
Excited states  satisfy the normalization condition % and the orthonormalization condition \eqref{orthoExcitatioOpDef1} implies 
...
can be found by employing techniques which can be mutuated from the ones adopted for GS calculations. Being expressed by localized excited states,
...
knowing that $\ket{f_p^\Phi(\omega)} =\sop \chi(\omega) \op \Phi(\omega) \ket{\psi_p}$, % \dm'_\Phi(\omega)
...
\caption{\label{co_AlphaExc}(Color online) Convergence of the statical polarizability $\alpha_{zz}$ of the $CO$ molecule, in a box of $11.5$ \AA,  w.r.t to excitations considered in the integral of Eq.~\eqref{sminus2}. The reference value obtained from Eq.~\eqref{staticalpha} is represented as a horizontal red dashed line. The value of the IP energy is indicated by a vertical black dashed line. For each excitation energy, the values of the oscillator strengths $w^a_{zz}$ of Eq.~\eqref{sminus2} are also plotted. }
\end{figure}
