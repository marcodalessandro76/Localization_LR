...
The bound states can be of course labeled by quantum numbers that are defined by the properties of the scatterer, namely the
...
\caption{(Color online) Benzene molecule. (a): Excitation Landscape: the excitations have been separated in discrete (black circle) and continuum (blue square) sectors following the criteria of Sec.~\ref{ExcitationLandscape}. (b): DOS of excitation energies $\Omega_a$. The first IP threshold is depicted as the vertical dashed line.}
\end{figure}
...
to find the correct result; the sector of the discrete excitations
%, even though parametrized by localized, observable (in the sense of reproducible) excited states,
appears to be largely insufficient to express the reference fluctuation state. In particular, in the case of $CO$, the contribution of localized excitations is even negligible w.r.t. the one coming from the continuum sector.
...
\includegraphics[scale=0.2]{Fig6b_C60_statPolvsExc.eps}}
\caption{\label{co_AlphaExc}(Color online) Convergence of the statical polarizability $\alpha_{zz}$ of the $CO$ (panel a) and $C_{60}$ (panel b) molecules,
w.r.t the excitations considered in the integral of Eq.~\eqref{sminus2}. The reference value obtained from Eq.~\eqref{staticalpha} is represented as a
horizontal red dashed line. The value of the IP energy is indicated by a vertical black dashed line. For each excitation energy, the values of the
oscillator strengths $w^a_{zz}$ of Eq.~\eqref{sminus2} are also plotted. }
\end{figure}
