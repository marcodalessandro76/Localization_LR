...
This fact implies that the degrees of completeness of the computational basis should therefore be increased by adding the necessary degrees
of freedom for describe the FS in all the relevant domain. Stated otherwise, in order to achieve precise results in in this regime the 
underlying basis set has to able to express delocalized states.
%The degrees of completeness of the computational basis should therefore be increased by imposing oscillatory degrees of freedom needed to 
%represent the FS at the border of the simulation domain: it is important that the underlying basis set is able to express delocalized states.
...
So far we have showed with both formal arguments and numerical calculations that the excitation spectrum split into two sectors with very distinct features.
...
To further address this point we exploit the formal relation between response density and susceptibility functional %established in ~\eqref{SusceptibilityDef1} 
$\dm'(\omega) = \sop \chi(\omega)\op\Phi(\omega)$ and make usage of the diadic representation ~\eqref{rhoPrimeFluctuationStateDef1} of the response density. Pursuing this procedure provides the excitation-based
...
% Such a locality is evident for the imaginary part of the FS, which can be straightforwardly restricted to the sum over $\ket{w_p^a}$ below IP (see Eq.~\eqref{imalpha}). For the real part of FS the \emph{entire} spectrum
% of excitations contribute.
The localization below threshold is manifest for the imaginary part of the FS, that is explicitly written as a the sum over $\ket{w_p^a}$ below IP (see Eq.~\eqref{imalpha}). Instead, for the real part of FS, the \emph{entire} excitation spectrum contributes to the construction of the FS and the localization
\emph{has} to realize due to a non trivial cancellation of the unbound parts of the $\ket{w_p^a}$'s.
...
continuum sector of the excitations can never be neglected}. In other terms, we \emph{cannot} exclude that a fluctuation state, even if genuinely localized, will have a nonzero projection in the essential part of the Liouvillian spectrum. This fact shows that it is \emph{impossible} to restrict the spectral representation of $\chi(\omega)$ to only localized, discrete, \emph{code-independent} excitations, even for representing LR results in the $\omega=0$ limit.
...
A given numerical treatment typically provides only an effective description of the sector of delocalized excitation which in turns give rise to an effective 
...
varying the molecule size and symmetry seems not to modify the qualitative behavior of the excitation landscape. In addition the
...
and the excitation density of states for $\omega >$ IP provided by this type of basis-set has to be considered as a effective representation of $\sop \chi(\omega)$ in order to fulfill Eq.~\eqref{fsinexc}. This fact has been pointed out several times (see e.g. \cite{giustino2012,giustino2014}) in the literature.
...
\dmnot \op O \op Q_0  - \op Q_0 \op O \dmnot  \;,
\end{equation}
consequently the Liouvillian superoperator, which contains by definition a commutator with $\hnot$ and $\dmnot$ in its bare and coupling part, is constructed such as $\Liouv \op O = \Liouv O_\perp$. In addition, the image operator satisfies the transverse condition
