...
\emph{open} (i.e. Isolated) Boundary Conditions (BC) that have to be imposed to the system will have on the numerical convergence of LR quantites.
In particular, we will discuss the interplay between the numerical convergence of LR quantities and the \emph{locality}
...
Our consideration will allso reveal useful for the understanding of the \emph{analytic structure} of LR quantities like
...
and we have introduced the \emph{response density operator} $\dm'(\omega)$ that codifies the modification of the ground-state density induced by the perturbation. 
...
%Due to this analysis we aim to be able to understand if the objects we are dealing with behave as bound-state like functions or if 
%This analysis allows us to establish comprehension of the type of objects one is dealing with when performing a LR calculation and 

%in determining the long range behavior, \emph{i.e.} outside the support domain of the potential. This analysis  
...
%asymptotic behavior of the main building blocks of the LR in the 
...
\left(\omega - \Liouv\right) \dm'(\omega) =  \commutator{\op\Phi(\omega)}{\dmnot} \;,
\ee
where $\op\Phi(\omega)$ represents the perturbing field. The inspection of the linear order time evolution of the eigenstates of $\hnot$ evidence that the response density acts as a transition 
...
operators so that $\op E_a = \dmnot\op E_a\op Q_0 + \op Q_0\op E_a\dmnot$. This condition allows us to introduce a dyadic representation of excitations as follows
...
\eqref{ExcitationOperatorsDef1} can be recasted in the form of a Sternheimer-like equations for the excited states, that is   
...
\end{align}
where $\op G_H$ is the Helmholtz Green function, while the (self-consistent) source term $s_\phi$ and $s_{\chi}$ collect the sum of the r.h.s of \eqref{ExcitationOperatorsDef3} 
plus the action of the KS potential on the excited states. 

It is possible to show that excitation eigenvalues come in pair and we focus our analysis on solutions with positive energy, \emph{i.e.} with $\Omega_a>0$. Looking at the coordinate representation 
of $\op G_H$ (see eq. \eqref{HelmholtzKernelDef1}) we observe that the Helmholtz kernel evaluated in $-\Omega_a+\eps_p$ contains an exponential damping factor for \emph{all} the positive values of 
$\Omega_a$. As a consequence all the $\ket{\chi^a_p}$ states exhibit a bound-state behavior in the long range. Instead, the asymptotic character of $\op G_H(\Omega_a+\eps_p)$ depends from $\Omega_a$ 
and we can identify the threshold value $\Omega_a=|\eps_p|$ in terms of which $\ket{\phi^a_p}$ changes from a bound-state like behavior to a delocalized spherical-wave like character.  
...
the corresponding energy levels. We conclude that all the states $\ket{\chi^a_p}$ and the $\ket{\phi^a_p}$ with $\Omega_a$ below the threshold value constitute a genuine \emph{discrete} set, while 
the ones in the unbound energy range behaves as generalized eigenvectors associated to a continuum spectrum. 
...
\ee 
with $\op W_a = \frac{1}{\sqrt{2}}(\op E_a + \op E_a^t)$. 
We have explicitly separated the contributions of the localized and continuous excitations in the spectral decomposition \eqref{RhopExcitationDef2}. 
...
\subsubsection*{Results of this section}
...
\appendix
\section{Left and right action of the Liouvillian superoperator}

% The Liouvillian superoperator for self-consistent systems is expressed as the sum of the unperturbed part plus a coupling term due the density dependence of the ground-state Hamiltonian.
% Its left action reads
% \begin{align}\lb{LiouvillianLeftActionDef1}
% \Liouv\op O &= \commutator{\hnot}{O} + \commutator{\op V'[\op O]}{\dmnot}  = \nn \\
% &=  \commutator{\hnot}{O} + \op Q_0\op V'[\op O]\dmnot - \dmnot\op V'[\op O]\op Q_0 \;
% \end{align}
% thanks to the equivalence $\commutator{\dmnot}{\op O} = \dmnot \op O \op Q_0 - \op Q_0 \op O \dmnot$. 
% It is also useful to define the scalar coupling kernel $U$
% \be
% U\left[\op O; \op O'\right] \equiv  \int \dd \r \dd \r' \trace{\op O \frac{\delta \hnot }{\delta \rho_0(\r,\r')}
% } \trace{\op O' \ket{\r} \bra{\r'}} \;, \nn
% \ee
% that allows us to express $\op V'[\op O]$ as:
% \be\lb{VpDef1}
% \op V'[\op O]= 
% \int \dd \r \dd \r' U\left[\ketbra{\r}{\r'};\op O\right] \ketbra{\r'}{\r}\;.
% \ee
% The right action of $\Liouv$ can be assessed through the equivalence $\trace{\op O'(\Liouv\op O)} = \trace{(\op O'\Liouv)\op O}$ and provides:
% \be
% \op O\Liouv = -\commutator{\hnot}{\op O} + \int \dd\r\dd\r'
% U\left[\commutator{\dmnot}{\op O};\ketbra{\r'}{\r}\right]\ketbra{\r}{\r'} \;. \nn
% \ee
% Using the symmetry of the coupling kernel we have
% \begin{align}%\lb{LiouvillianRightActionDef1}
% \op O\Liouv &= -\commutator{\hnot}{\op O} + \nn \\ 
% &+ \dmnot\op V'\left[\commutator{{\dmnot}}{\op O}\right]\op Q_0 + \op Q_0\op V'\left[\commutator{\dmnot}{\op O}\right]\dmnot 
% \end{align}


The Liouvillian superoperator $\Liouv$ for self-consistent systems is expressed as the sum of the unperturbed part plus a coupling term due
the density dependence of the ground-state Hamiltonian. The left action of these terms reads, respectively
\be\lb{LiouZeroDef1}
...
\ee
Here $\op V'$ can be conveniently expressed by defining the \emph{scalar coupling kernel}
\be
U\left[\op O; \op O'\right] \equiv  \int \dd \r \dd \r' \trace{\op O \frac{\delta \hnot }{\delta \rho_0(\r,\r')}
} \trace{\op O' \ket{\r} \bra{\r'}} \;, \nn
\ee
...
U\left[\commutator{\dmnot}{\op O};\ketbra{\r'}{\r}\right]\ketbra{\r}{\r'} \;. \nn
\ee
In the cases relevant for the present analysis the coupling kernel is symmetric with respect to the exchange of its arguments so
\begin{align}\lb{LiouvillianRightActionDef1}
\op O\Liouv &= -\commutator{\hnot}{\op O} + V'\left[\commutator{{\dmnot}}{\op O}\right] = -\commutator{\hnot}{\op O} + \nn \\ 
&+ \dmnot\op V'\left[\commutator{{\dmnot}}{\op O}\right]\op Q_0 + \op Q_0\op V'\left[\commutator{\dmnot}{\op O}\right]\dmnot \;, 
\end{align}
where in the last equivalence we have made usage of the transverse property of the coupling superoperator. Formula \eqref{LiouZeroDef1},\eqref{CouplDef1}
and \eqref{LiouvillianRightActionDef1} are the starting point for the solution of the eigenvalues equations \eqref{ExcitationOperatorsDef1}. 
