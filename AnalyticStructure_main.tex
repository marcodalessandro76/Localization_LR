...
that the Isolated BC have on the solutions of the time-independent Schr\"odinger Equation.}{Let us start our discussion by revisiting the 
...
\added[id=dal]{Fluctuation states are complex quantities and consequently $\dm'_\Phi(\omega)$ is not hermitian; nonetheless the $\omega$ dependence of \eqref{rhoPrimeFluctuationStateDef1} ensures that the hermitian nature of this operator is recovered in the time domain, as expected.} 
...
\added[id=dal]{Added a subsection for static perturbations}
...
\added[id=dal]{Modified the name of the subsection}
...
\ee
\added[id=dal]{Here add a short justification for the expression of the real and imaginary part of the linear response functional written
in terms of excitation operators. Principal part vs delta functions.}

\added[id=dal]{Modified name of the subsection}
...
\begin{itemize}
\item It is composed by a genuine set of discrete elements. 
\item The values of the excitation energies do not depend on the computational setup (observable features) and so can be compared among different approaches. 
\item The knowledge of this sector of the excitation spectrum allows us to express only the imaginary part of (proper equation that expresses the response functional written using Sokowsky-Plemely
on the susceptibility functional). 
\item A computational setup in which delocalized degrees of freedom are excluded by design can in principle describe to this sector in a exact way. 
\end{itemize}

\subsection{Continuum sector}

\begin{itemize}
 \item Not true poles of the susceptibility (but only an effective tool to compute the spectral function).
 \item Deeply affected by the computational setup used to represent them. In particular, the presence of a finite computational domain implies a fictitious discretization of the excitation levels and the associated spectrum exhibits an explicit dependence from the size of the domain.
 \item This sector of the excitation spectrum is needed to compute imaginary parts above IP and real part for \emph{all} the values of $\omega$. Add examples on the statical polarizability. Can we comment
