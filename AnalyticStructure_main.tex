...
\ee
with the normalization condition $\trace{\op{\tilde E}\op E'} = \delta(E-E')$. The properties of the Liouvillian superoperator (see appendix?) imply that excitations are transverse 
...
\end{align}
and the \emph{excited states} $\ket{\phi^E_p}$ and $\bra{\tilde\chi^E_p}$ and their conjugates span the domain of $\op Q_0$, \emph{i.e} the empty subspace of $\hnot$. 

We restrict our analysis assuming that a set of hypothesis commonly realized in DFT holds (symmetry of the transition basis, specify better? see appendix?). In this case excitation
eigenvalues come in pair. It is possible to show that and if a couple of left and right eigenstates satisfy \eqref{ExcitationOperatorsDef1} with energy $E$ than their transpose 
%$(\op E,\op{\tilde E})$ $(\op E^t,\op{\tilde E}^t)$ 
are eigenstates of the same equations with eigenvalue $-E$. Equations \eqref{ExcitationOperatorsDef1} can be recasted in the form of a Sternheimer-like equations for the excited states. 
In particular, the left action eigenvalue equation provides
%We further analyze the first equation of \eqref{ExcitationOperatorsDef1} under a.   
%In this case the excitation eigenvalues come in pair and for each 
%that can be recasted in the form of a Sternheimer-like equations for the excited states. We assume that
%the condition $(\op V'[\op E])^t=\op V'[\op E^t]$
%The eigenvalue 
%condition \eqref{ExcitationOperatorsDef1} 
%can thus be recasted in the 
% \begin{align}
% &\left(\hnot - \eps_p\right) \ket{\phi_p^E} + \op Q_0 \op V'[\op E] \ket{\psi_p} = E \ket{\phi_p^E} \;, \nn\\
% &\left(\hnot - \eps_p\right)\ket{\tilde\chi_p^E} + \op Q_0\op V'[\op E^t]\ket{\psi_p}   = -E\ket{\tilde\chi_p^E}  \;, 
% \end{align}
...
\end{align}
where $\op G_H$ is the Helmholtz Green function and the (self-consistent) source term $s_\phi$ and $s_{\tilde\chi}$ collect the sum of the r.h.s of \eqref{ExcitationOperatorsDef3} 
plus the action of the KS potential on the excited states. 

We focus our analysis on solutions with positive energy, \emph{i.e.} with $E>0$. Looking at the coordinate representation of $\op G_H$ (see eq. \eqref{HelmholtzKernelDef1}) we conclude
that the Helmholtz kernel evaluated in $-E+\eps_p$ contains an exponential damping factor for positive values of $E$. As a consequence all the $\ket{\tilde\chi^E_p}$ states
exhibit a bound-state behavior in the long range. Instead, the asymptotic character of $\op G_H(E+\eps_p)$ depends from the value of $E$ and $\ket{\phi^E_p}$ varies from a bound-state 
like behavior for $E<|\eps_p|$, to a delocalized spherical-wave like character for values of $E>|\eps_p|$. 
%On the contrary, the localization character of $\ket{\phi^E_p}$ varies from bound-state like solutions, for $E<|\eps_p|$, to delocalized spherical-wave like 
%behavior for values of $E>|\eps_p|$. 
There is however a novel aspect which did not emerged in the analysis of fluctuation states and is determined by the presence of the further condition \eqref{ExcitedStateOrthNormDef1}.
...
Excited states with $E<|\eps_p|$ constitute a genuine \emph{discrete} set while the ones in the unbound energy range $E>|\eps_p|$ belong to a continuum. 

This considerations determine the structure of the excitations spectrum and essentially evidence that there exists two distinct sectors of the exctiation spectrum. The first one is composed by
