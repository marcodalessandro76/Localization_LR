...
In this paper, we focus our attention on Linear-Response treatments of \emph{molecular} systems,
...
can be effectively restricted to wavefunctions that can be considered as nonzero only on a
...
which are tailored to express asymptotically vanishing wavefunctions are better suited for the solution of the problem.
...
the Helmholtz kernel behaves as a spherical wave and  gives raise to a $\ket{f^\Phi_p(\omega)}$ which is \emph{delocalized} over the entire real-space domain.
...
of a $CO$ molecule as a function of the size of the computational domain. The inset evidences the relative convergence (in percent) of the two quantities w.r.t. the corresponding value associated with the largest
box.}
\end{figure}
...
If any of these hypotheses does not hold, the excitations of the systems are defined via genuinely delocalized wavefunctions.
...
\end{tikzpicture}
\caption{\label{ExcitationLandscape} Excitation landscape. Excitations are split in horizontal lines according to the threshold levels $p$. Filled bullets refer to localized
...
{\label{C6H6_exc}
\includegraphics[scale=0.58]{Fig5b_C6H6_dos_rev1.eps}}
\caption{(Color online) Benzene molecule. (a): Excitation Landscape: the excitations have been separated into discrete (black circle) and continuum (blue square) sectors following the criteria of Sec.~\ref{ExcitationLandscape}. (b): DOS of excitation energies $\Omega_a$. The first IP threshold is depicted as the vertical dashed line.}
\end{figure}
...
%In particular, in the case of $CO$, the contribution of localized excitations is even negligible w.r.t. the one coming from the continuum sector.
...
\subfloat[$CO$]
{\label{CO_alphavsExc}
\includegraphics[scale=0.6]{Fig6a_CO_statPolvsExc.eps}} \\
\centering
\subfloat[$C_{60}$]
{\label{C60_alphavsExc}
\includegraphics[scale=0.6]{Fig6b_C60_statPolvsExc.eps}}
...
This scenario is confirmed by the results for the other molecules presented in Table \ref{alphaTable};
varying the molecule size and symmetry does not modify the qualitative behavior of the excitation landscape (for further details
about the dependence of the excitation landscape on the size of the molecule see the supplementary material). In addition the
localized sector of the excitations alone is largely insufficient to express $\alpha_{ij}$.
Such considerations are independent of the number of atoms of the molecule, as the range in $\omega$ needed to
...
All the details of the calculations, the \textsl{Jupyter} notebooks of the data analysis, including the supplementary information, may be found at the URL~\footnote{\texttt{https://github.com/luigigenovese/LR-nb/blob/master/ POLARIZABILITY/nb\_paper/LR\_Analysis.ipynb}, or \texttt{https://gitlab.com/luigigenovese/ localization-of-excitations/ blob/master/LR\_Analysis.ipynb}}
